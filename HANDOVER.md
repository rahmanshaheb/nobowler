...
└── handover/                     # NOT committed to git — see Database Handover below
    ├── database_dump_*.dir.tar.gz  # Full pg_dump -Fd archive from production (preferred)
    └── database_export_*.sql       # Legacy INSERT export from server/scripts/export-data.js
```
...
1. Set up your own local (or your own hosted) PostgreSQL instance (**PostgreSQL 18+** client tools if using the directory dump).
2. Place the handover archive in `handover/` (e.g. `database_dump_2026-08-20.dir.tar.gz`).
3. Restore:
   ```bash
   brew install postgresql@18   # pg_restore must match dump version
   export PATH="/opt/homebrew/opt/postgresql@18/bin:$PATH"
   bash server/scripts/restore-handover-db.sh
   ```
   Or run `bash local-dev-setup.sh` and accept the handover restore prompt.

   **Preferred format:** `database_dump_*.dir.tar.gz` — a `pg_dump -Fd` archive from production (schema + data). The restore script recreates the `nobowlers` database automatically.

   **Legacy format:** `database_export_*.sql` — plain INSERT export; the script applies `db/schema.sql` + `db/local_dev_migrations.sql` first, then loads data.

4. For SQL-only exports: manually insert one row each into `admin_account` and `scorer_passcode` if those tables are empty — they were **deliberately excluded** from INSERT exports since they hold real login credentials. The full pg_dump archive includes production credentials; rotate them locally or run `npm run seed` in `server/` after restore to set dev defaults.

**Refreshing handover data:** take a new Render backup (`pg_dump -Fd`) and save as `handover/database_dump_<date>.dir.tar.gz`, or run `node server/scripts/export-data.js` for a legacy SQL export. The `handover/` directory is gitignored — share dumps directly (AirDrop, private link), don't commit them.
